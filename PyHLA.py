...
def hlaFreq(caseAlleles, ctrlAlleles, np, nc, nn):
    ...
    '''
    temp = []
    temp.extend(list(caseAlleles.keys()))
    temp.extend(list(ctrlAlleles.keys()))
...
def printAssocRaw(assoc, alleles, permP=None, permN=None, permNA=None):
    ...
    for a in assoc:
        print("%10s" % a, end='')
        print("%10.4f" % assoc[a][0], end='')
        print("%10d" % assoc[a][1], end='')
        if assoc[a][2] == 'NA':
                print("%10s" % 'NA', end='')
        elif assoc[a][2] > 0.001:
            print("%10.4f" % assoc[a][2], end='')
        else:
            print("%10.2e" % assoc[a][2], end='')
        if permP is None:
            print()
        else:
            if permP[a] > 0.001:
...
def printAssocDelta(assoc, perm=None):
    ...
    for a in sorted(assoc.keys()):
        ...
        print("%10.4f" % assoc[a][2], end='')
        if assoc[a][3] > 0.001:
            print("%10.4f" % assoc[a][3], end='')
        else:
            print("%10.2e" % assoc[a][3], end='')
        if perm is None:
            print()
        else:
            if assoc[a][4] > 0.001:
...
def adjustP(pvalues, method = "Benjamini-Hochberg"):
    ...
    if method == "Bonferroni":
        cp = [min(x*n,1.0) for x in pvalues]
...
def assocADRChiFisher(infile, digit, freq, test='chisq', model = 'allelic', adjust='FDR', exclude=None, perm=None, seed=None):
    ...
    usedAllele = []
    assoc = {}
    for allele in list(caseAlleles.keys()):
        if allele in ctrlAlleles:
    ...
    ### adjust
    genes = []
    for g in list(np.keys()):
        if g in list(nc.keys()):
            genes.append(g)
...
def assocDelta(infile, digit, freq=0.05, adjust='FDR', exclude=None, perm=None, seed=None):
    ...
    ### adjust
    genes = []
    for g in list(popP.keys()):
        if g in list(popC.keys()):
            genes.append(g)
    for g in sorted(genes):      ### GENE BY GENE
        ps = []
        ns = []
...
def printAAA(assoc):
    ...
    for k in sorted(assoc.keys()):
        print("%-20s" % (k[0] + '_' + str(k[1]) + '_' + k[2]), end='')
        for i in range(4):
...
if usedtime >=60:
    ...
    th = int(usedtime) % 60
    if th > 0:
        print("%d hours"  % th, end='')
        print("%d minutes"  % tm, end='')
    elif tm > 0:
        print("%d minutes"  % tm, end='')
else:
    ts = usedtime
